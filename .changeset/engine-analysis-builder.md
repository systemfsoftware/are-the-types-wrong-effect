...
Failures are values: `ManifestUnreadable`, `CompilerFailed`, and `LexerUnavailable` arrive on the error channel. `withModes` is gone — every analysis covers all four resolution kinds — and so are `CheckResult`, the `PackageStore` services, and `parsePackageSpec`; bring a `Package` from `@systemfsoftware/npm-package`. `pos`, `end`, `resolutionMode`, and `visibleProblems` now decode as natural numbers.
